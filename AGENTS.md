...
`app.R` is the single entry point. It sources, in order: `R/utils.R`, `R/_setup.R`, `R/echarts_helpers.R`. The theme is `bs_theme(version = 5, brand = TRUE)` — Ekio colors and typography come from `_brand.yml` (requires the `brand.yml` package), which names Host Grotesk as the base font. The faces are self-hosted from `fonts/` (latin-subset woff2, SIL OFL) and registered by `fonts/host-grotesk.css` through the `host_grotesk` htmlDependency in `app.R` — brand.yml's `source: file` emits only the first file, so weights 500–700 would otherwise be synthesized. `echarts4r::e_common(font_family = )` gives every chart (canvas text and tooltips) the same font; the regular face is preloaded because ECharts does not redraw when a font arrives late. `styles.css` adds the layout rules on top of the theme.
...
- `R/pipeline/loader.R` implements `load_dataset(name, update = FALSE)`: fetch → prepare → stamp `fetched_at` and `pipeline_version` → cache. It upgrades older caches in memory and preserves the last good cache for incremental SGS fetches and transient upstream failures.
...
- Ekio branding follows the ekioplot brand scales: navy `#1E3A5F` (blue.700) primary, orange `#D3742A` (orange.400) for selection and the latest reading, teal `#006261` only in chart series, off-white `#FBFBF6` page, ekio_brand Soft Linen `#F2EDE2` for the lead KPI — defined in `_brand.yml` and as CSS variables in `styles.css`
- Layout is a ruled bulletin, not a card wall: `kpi_board(lead, ...)` puts the first KPI in a wide linen column beside a hairline ledger; charts sit on `.chart-plate` rules with no shadows or radii; the active page, the checked filter option and each sparkline's latest bar share one orange marker
