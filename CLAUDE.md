...
Dependencies are pinned with **renv** (`renv.lock`); `.Rprofile` auto-activates the project library. On a fresh clone run `renv::restore()` to install the recorded versions. `realestatebr` is on CRAN, recorded as a normal Repository package (it was previously GitHub-sourced). `brand.yml` — used implicitly by `bs_theme(brand = TRUE)` — is referenced via a `requireNamespace()` guard at the top of `app.R` so renv tracks it. After adding a package, run `renv::snapshot()`.
